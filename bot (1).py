...
def build_summary(data, lang):
    ...
    til = "O'zbek 🇺🇿" if lang == "uz" else "Rus 🇷🇺"
    polis = "✅ Ha" if data.get("owner_in_policy") else "❌ Yoq"
    ...
    techpass = data.get('techpass', '—')
    owner_passport = data.get('owner_passport', '—')
    owner_jshir = data.get('owner_jshir', '—')
    owner_birth = data.get('owner_birth', '—')
    ...
    return (
        ...
        f"🚗 *MASHINA*\n"
        f"Raqam: `{plate}`\n"
        f"Texpassport: `{techpass}`\n\n"
        f"🛡️ *SUG'URTA TURI:* {ins}\n\n"
        f"👤 *EGASI*\n"
        f"Pasport: `{owner_passport}`\n"
        f"JSHIR: `{owner_jshir}`\n"
        f"T.sana: {owner_birth}\n"
        f"Polis ichida: {polis}"
        f"{haydovchilar}\n"
        f"📆 *MUDDAT:* {dur}\n"
        f"💳 *TO'LOV:* {pay}\n"
        f"━━━━━━━━━━━━━━━\n"
